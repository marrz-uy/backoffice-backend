...
RUN sed -i echo 'extension=/opt/remi/php81/root/usr/lib64/php/modules/ldap.so' >> /etc/opt/remi/php81/php.d/50-ldap.ini
...
RUN composer update && cp .env.example .env && php artisan key:generate && php artisan optimize
